    ...
    class Scan(PoolScanner):
        ...
        def object_action(self,buff,object_offset):
            ...
            if not outfd == None:
                ...
                a = "0x%08x" % (address)
                o = "0x%08x" % (ObjType)
                s = "0x%08x" % (DriverStart)
                if ServiceKey == None:
                    ServiceKey = ''
                cur.execute("insert into driverscan2 values (?,?,?,?,?,?,?,?,?,?)",
                            (a, o, Pointers, Handles, s, DriverSize, ServiceKey.lower(), Name.lower(), DriverName.lower(), imgname))
                conn.commit()
                conn.close()

class driverscan2sql(forensics.commands.command):
    ...
    def execute(self):
        ...
        if not opts.outfd1 == None:
            ...
            try:
                cur.execute("select * from driverscan2")
            except sqlite3.OperationalError:
                cur.execute("create table driverscan2(paddr text, objtype text, pointers integer, handles integer, start text, size text, srvckey text, driver text, path text, memimage text)")
                conn.commit()
            conn.close()
        else:
            outfd = None
